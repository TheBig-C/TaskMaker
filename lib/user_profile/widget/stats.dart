...
class TaskStatisticsWidget extends StatelessWidget {
      ...
      return FutureBuilder<List<Task>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            Map<TaskCategory, int> categoryCounts = calculateMostUsedCategories(tasks);
            Map<String, int> completionStatus = calculateCompletionStatus(tasks);
            double averageTimePerTask = calculateAverageTimePerTask(tasks);
            Map<String, int> taskDistribution = calculateTaskDistributionByDate(tasks);
  ...
  Future<List<Task>> _fetchFilteredTasks(CollectionReference collection, String userId) async {
  ...
  return SfCartesianChart(
    ...
    series: <CartesianSeries<TaskCategory, String>>[
      ColumnSeries<TaskCategory, String>(
        ...
        xValueMapper: (TaskCategory category, _) => category.toString().substring(1),
        yValueMapper: (TaskCategory category, _) => categoryCounts[category],
        dataLabelSettings: DataLabelSettings(isVisible: true), // Mostrar valores encima de las barras
      ),
    ],
  ...
  return SfCircularChart(
    series: <CircularSeries>[
      PieSeries<MapEntry<String, int>, String>(
        ...
        dataLabelSettings: DataLabelSettings(isVisible: true), // Mostrar valores dentro del pastel
      ),
    ],
    legend: Legend(isVisible: true), // Agregar leyenda al gráfico
  );
